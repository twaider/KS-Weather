...
static TextLayer *s_weather_layer, *s_weathertext_layer;
static GFont s_weather_font, s_icon_font;
static GPoint s_center;
static Time s_last_time, s_anim_time;
static int s_radius = 0, s_color_channels[3];
static int background_color;

static bool s_animating = false, weather_units_conf = false,
            weather_safemode_conf = true, weather_on_conf = false,
...
static void setRandomColor() {
  ...
  int toHex(int r, int g, int b) { return (r << 16) | (g << 8) | b; }

  background_color =
      toHex(s_color_channels[0], s_color_channels[1], s_color_channels[2]);
}
...
static void tick_handler(struct tm *tick_time, TimeUnits changed) {
  ...
  if (tick_time->tm_min % 5 == 0 && !background_on_conf) {
    setRandomColor();
  }
...
static void update_proc(Layer *layer, GContext *ctx) {
  ...
  GPoint minute_hand = (GPoint){
      .x = (int16_t)(sin_lookup(TRIG_MAX_ANGLE * mode_time.minutes / 60) *
                     (int32_t)(s_radius - HAND_MARGIN) / TRIG_MAX_RATIO) +
           s_center.x,
  ...
  GPoint hour_hand = (GPoint){
      .x = (int16_t)(sin_lookup(hour_angle) *
                     (int32_t)(s_radius - (2 * HAND_MARGIN)) / TRIG_MAX_RATIO) +
           s_center.x,
